...
def process_data(df,file_name):
    ...
    for i in range(num_rows):#这里要注意从第几行开始算，如果第一行没有标题的就填-1，有标题填0，不然会落，这里python索引是从0开始算第一行，所以要-1
        ...
        if type(cell_value) == float:
            ...
            row = pd.DataFrame({'File':file_name, 'Row':int(i), 'Trial':None, 'Error': "Prompt too long", '原告': None, '原告性别': None, '被告': None, '被告性别': None, '被告是否胜诉': None, '判断的原因':None}, index=[0])
            ...
            for trial in range(5):
                ...
                try:
                    ...
                    logger_b.debug(f"Response: {response} \n")
...
def process_data_32k(df):
    ...
    for i in range(num_rows):
        cell_value = df.iloc[i,'cellvalue']
