...
bsc_prpt="标题：{}\n文书：{}\n请你根据这段裁判文书判断这个文书是否是离婚相关，以及是否因涉及未成年人而未公开，（并说明原因），回答格式为markdown表格代码，是否为离婚文书：，是否涉及未成年而未公开：，这么判断的原因： "# basic prompt
...
# set up
import logging, os, subprocess, pickle, pandas as pd

df_format = pd.DataFrame(columns=["File","Row","Trial","Error", "离婚", "未成年不公开", "判断的原因"])
...
def format_output(output):
    try:
        ...
        divorce = data[1].strip()
        children_protect = data[2].strip()
        reason = data[3].strip() if len(data) > 7 else "Data not responded"

        return divorce, children_protect, reason
